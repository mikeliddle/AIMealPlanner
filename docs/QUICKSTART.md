...
After generation, your plan goes to a staging area where you can:

- Review the selected recipes for each day of the week
- Swap any recipe if you prefer something different
- View the grocery list (updates automatically when you swap)

### 5. Accept Your Plan

Once satisfied:

- Optionally check "Add meals to Google Calendar" (requires setup)
- Click "Accept Meal Plan"
- Your plan is now active and saved

### 6. View Meal Plan & Grocery List
...
- Calendar sync status (if enabled)
...
To automatically add your meal plans to Google Calendar:

1. **Set up Google Cloud credentials** (one-time setup)
   - Follow the detailed guide in [docs/CALENDAR.md](CALENDAR.md)
   - Create OAuth credentials from Google Cloud Console
   - Save credentials as `data/credentials.json`

2. **First-time authorization**
   - When accepting your first meal plan with calendar sync enabled
   - A browser window will open for authorization
   - Grant the app access to your calendar
   - You'll only need to do this once

3. **Using the feature**
   - In the staging area, check "Add meals to Google Calendar"
   - Click "Accept Meal Plan"
   - Each meal is added as a calendar event at 6:00 PM

For detailed instructions, see [docs/CALENDAR.md](CALENDAR.md)
...
1. **Generate** - Create plan with AI-optimized recipe selection
2. **Stage** - Review in staging area before committing
3. **Customize** - Swap recipes you don't want this week
4. **Accept** - Finalize and optionally sync to Google Calendar
5. **Archive** - Move old plans to archive when done
...
- Updates automatically when you swap recipes in staging area
...
- `data/credentials.json` - Google OAuth credentials (optional, not included)
- `data/token.json` - Google auth token (auto-generated, not included)
